...
def init_db():
    ...
    conn.executescript("""
        ...
        CREATE INDEX IF NOT EXISTS idx_ua_tid ON user_activity(telegram_id);
...
def gen_key_code():
    ...
    return f"TOP1-{p1}-{p2}-{p3}"

def gen_api_key():
    return "TOP1-api_" + secrets.token_hex(16)
...
PROXY_SCRIPT = os.path.join(BOT_DIR, "proxy.py")
...
def proxy_status(port: int) -> str:
    pid = _read_pid(port)
    return "active" if _is_running(pid) else "inactive"
...
def proxy_action(port: int, action: str) -> bool:
    ...
    if action in ("start", "restart"):
        env = os.environ.copy()
        env["M3SB_ACTIVE_FEATURE"] = feature
        env["M3SB_DB_PATH"]        = DB_PATH
        env["M3SB_LOG_DIR"]        = LOG_DIR
        env["M3SB_DATA_DIR"]       = DATA_DIR
        mitmdump = os.path.join(BOT_DIR, "venv", "bin", "mitmdump")
        cert_dir = os.path.join(BOT_DIR, "certs")
        proc = subprocess.Popen(
            [mitmdump, "-s", PROXY_SCRIPT, "-p", str(port),
             "--mode", "regular",
             "--set", f"confdir={cert_dir}"],
            env=env,
            stdout=open(os.path.join(LOG_DIR, f"proxy_{port}.log"), "a"),
            stderr=subprocess.STDOUT,
        )
        with open(_pid_file(port), "w") as f:
            f.write(str(proc.pid))
        time.sleep(2)
        return _is_running(proc.pid)
    return False
...
# خريطة callback → اسم بشري للمسار
ACTION_LABELS: dict[str, str] = {
    "main_menu":        "🏠 الرئيسية",
    "gen_key":          "🔑 توليد مفتاح",
    "list_keys":        "📋 عرض المفاتيح",
    "my_keys":          "📋 مفاتيحي",
    "list_ips":         "🌐 IPs نشطة",
    "revoke_ip":        "❌ حذف IP",
    "ban_key":          "🚫 حظر مفتاح",
    "delete_key":       "🗑 حذف مفتاح",
    "add_reseller":     "👥 إضافة موزّع",
    "list_resellers":   "📜 قائمة الموزعين",
    "remove_reseller":  "🚫 حذف موزّع",
    "stats":            "📊 إحصائيات",
    "my_stats":         "📊 إحصائياتي",
    "bulk_key":         "📦 توليد مجموعة",
    "api_keys":         "🔐 مفاتيح API",
    "proxy_control":    "🖥 التحكم بالبروكسي",
    "convert_file":     "📁 تحويل ملف",
    "check_key":        "🔍 فحص مفتاح",
    "activate_key":     "✅ تفعيل مفتاح",
}

def track_activity(uid: int, action: str):
    """يسجّل حركة المستخدم داخل البوت (آخر 200 سجل فقط)."""
    try:
        ...
        # احتفظ بآخر 200 سجل لكل مستخدم
        conn.execute(
            "DELETE FROM user_activity WHERE id IN ("
            "  SELECT id FROM user_activity WHERE telegram_id=?"
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(uid):
        ...
        cert_url = "https://becd7f3b-a7ca-4aaa-bfbb-fcb80950847b-00-67irwzpet170.worf.replit.dev/api/cert"
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("🛡️ تثبيت شهادة البروكسي", url=cert_url)
        ]])
        await update.message.reply_text(
            "👋 *مرحباً بك في Proxy Top1*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "أرسل مفتاحك لتفعيل الوصول.\n"
            "_مثال:_ `TOP1-XXXXX-XXXXX-XXXXX`\n\n"
            "⚠️ *قبل الاستخدام:* ثبّت شهادة البروكسي على جهازك:",
            parse_mode="Markdown",
            reply_markup=kb,
        )
...
async def reset_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        await update.message.reply_text(
            "⚠️ Usage: `/reset TOP1-XXXXX-XXXXX-XXXXX`",
            parse_mode="Markdown",
        )
        return
...
async def check_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text(
            "⚠️ Usage: `/check TOP1-XXXXX-XXXXX-XXXXX`",
            parse_mode="Markdown",
        )
        return
    key_code = ctx.args[0].strip()
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # سجّل حركة المستخدم العادي فقط (ليس الأدمن)
    if not owner:
        # نأخذ الجزء الأول من callback كـ action
        _base = data.split("|")[0]
        track_activity(uid, _base)
    ...
    # ── Ban Key ──
        ...
        await q.edit_message_text(
            "🚫 *Ban Key*\n━━━━━━━━━━━━\n"
            "Send the key code:\n_Example:_ `TOP1-XXXXX-XXXXX-XXXXX`",
            parse_mode="Markdown", reply_markup=back_kb(),
        )
    ...
    # ── Add Reseller ──
        ...
        try:
            await ctx.bot.send_message(
                chat_id=rs_id,
                text=(
                    "🎉 *You are now a Reseller on Proxy Top1*\n"
                    "━━━━━━━━━━━━━━━━━━━━\n"
    ...
    # ── API Keys ──
        ...
        await q.edit_message_text(
            "🔐 *API Key Management*\n━━━━━━━━━━━━━━━━\n"
            "Manage API keys for `proxytop1.com`:",
            parse_mode="Markdown",
        ...
        await q.edit_message_text(
            ...
            f"_Base URL:_ `https://proxytop1.com/api/v1`",
            parse_mode="Markdown",
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Admin text actions ──
    if owner and action in ("admin_search_user", "admin_broadcast"):
        await handle_admin_text(update, ctx, uid, state)
        return
    ...
    if text.startswith("TOP1-"):
        ...
        if not row:
            await update.message.reply_text(
                ...
                "_Contact the admin to get a key._",
                parse_mode="Markdown",
            )
            return
        if row["status"] == "banned":
            await update.message.reply_text(
                "🚫 *Key Banned*\n_Contact the admin for support._",
                parse_mode="Markdown",
            )
            return
        if row["status"] == "used":
...
def admin_menu_kb():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👥 المستخدمين",    callback_data="admin_users|0"),
            InlineKeyboardButton("🔍 بحث مستخدم",    callback_data="admin_search_user"),
        ],
        [
            InlineKeyboardButton("🗺 مسار المستخدمين", callback_data="admin_activity|0"),
...
async def admin_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_owner(uid):
        ...
        return
    conn = get_db()
    total   = conn.execute("SELECT COUNT(*) FROM bot_visitors").fetchone()[0]
    today   = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE last_seen >= datetime('now','-1 day')").fetchone()[0]
    week    = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE last_seen >= datetime('now','-7 days')").fetchone()[0]
    conn.close()
    await update.message.reply_text(
        "👑 *لوحة الإدارة — Proxy Top1*\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
...
async def admin_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_owner(uid):
        await q.edit_message_text("⛔ للمالك فقط.")
        return
    ...
    if data.startswith("admin_users|"):
        ...
        total   = conn.execute("SELECT COUNT(*) FROM bot_visitors").fetchone()[0]
        rows    = conn.execute(
            "SELECT v.telegram_id, v.username, v.first_name, v.last_seen, "
            "       k.key_code, k.used_at "
            "FROM bot_visitors v "
            "LEFT JOIN license_keys k ON k.used_by_uid = v.telegram_id "
            "ORDER BY v.last_seen DESC LIMIT ? OFFSET ?",
            (limit, offset)
        ).fetchall()
        conn.close()

        if not rows:
            await q.edit_message_text("👥 لا يوجد مستخدمون.", reply_markup=back_kb("admin_menu"))
            return
        ...
        for r in rows:
            ...
            seen    = r['last_seen'][:10] if r['last_seen'] else "—"
            key_str = f"`{r['key_code']}`" if r['key_code'] else "❌ بدون مفتاح"
            lines.append(
                f"👤 {name} | {uname}\n"
                f"🆔 `{r['telegram_id']}`\n"
                f"🔑 {key_str}\n"
        ...
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("◀️ السابق", callback_data=f"admin_users|{page-1}"))
        if offset + limit < total:
            nav.append(InlineKeyboardButton("التالي ▶️", callback_data=f"admin_users|{page+1}"))

        kb = []
        if nav:
            kb.append(nav)
        kb.append([InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_menu")])

        await q.edit_message_text(
            f"👥 *المستخدمون* — صفحة {page+1} من {((total-1)//limit)+1} (إجمالي: {total})\n"
            "━━━━━━━━━━━━━━━━\n\n" + "\n\n".join(lines),
            parse_mode="Markdown",
        ...
        pending[uid] = {"action": "admin_search_user"}
        await q.edit_message_text(
            "🔍 *بحث عن مستخدم*\n━━━━━━━━━━━━\n"
        ...
        today   = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE last_seen >= datetime('now','-1 day')").fetchone()[0]
        week    = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE last_seen >= datetime('now','-7 days')").fetchone()[0]
        month   = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE last_seen >= datetime('now','-30 days')").fetchone()[0]
        new_today = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE first_seen >= datetime('now','-1 day')").fetchone()[0]
        new_week  = conn.execute("SELECT COUNT(*) FROM bot_visitors WHERE first_seen >= datetime('now','-7 days')").fetchone()[0]
        active_ips = conn.execute("SELECT COUNT(*) FROM allowed_ips WHERE status='active' AND expires_at > ?", (int(time.time()),)).fetchone()[0]
        total_keys = conn.execute("SELECT COUNT(*) FROM license_keys").fetchone()[0]
        used_keys  = conn.execute("SELECT COUNT(*) FROM license_keys WHERE status='used'").fetchone()[0]
        conn.close()
        await q.edit_message_text(
            ...
            f"     اليوم      : `{new_today}`\n"
            f"     الأسبوع    : `{new_week}`\n\n"
            f"🌐  IPs نشطة   : `{active_ips}`\n"
            f"🔑  مفاتيح مستخدمة: `{used_keys}/{total_keys}`",
            parse_mode="Markdown",
            reply_markup=back_kb("admin_menu"),
        )
    ...
    # ── مسار المستخدمين ──────────────────────────────────────────
    elif data.startswith("admin_activity|"):
        page   = int(data.split("|")[1])
        limit  = 15
        offset = page * limit
        conn   = get_db()
        ...
        rows   = conn.execute(
            "SELECT a.telegram_id, v.username, v.first_name, a.action, a.ts "
            "FROM user_activity a "
            "LEFT JOIN bot_visitors v ON v.telegram_id = a.telegram_id "
        ...
        for r in rows:
            name  = r['first_name'] or "—"
            uname = f"@{r['username']}" if r['username'] else f"`{r['telegram_id']}`"
            ts    = r['ts'][11:16] if r['ts'] else "—"  # HH:MM
        ...
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("◀️ السابق", callback_data=f"admin_activity|{page-1}"))
        if offset + limit < total:
            nav.append(InlineKeyboardButton("التالي ▶️", callback_data=f"admin_activity|{page+1}"))
        ...
        await q.edit_message_text(
            f"🗺 *مسار المستخدمين* — صفحة {page+1}/{total_pages} (إجمالي: {total})\n"
            "━━━━━━━━━━━━━━━━\n\n" + "\n\n".join(lines),
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(kb),
        )
...
async def handle_admin_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE, uid: int, state: dict) -> bool:
    """Returns True if handled by admin logic."""
    text   = update.message.text.strip()
    action = state.get("action")
    ...
    if action == "admin_search_user":
        ...
        conn = get_db()
        if text.isdigit():
            row = conn.execute("SELECT * FROM bot_visitors WHERE telegram_id=?", (text,)).fetchone()
        else:
            uq = text.lstrip("@")
            row = conn.execute("SELECT * FROM bot_visitors WHERE username=?", (uq,)).fetchone()
        if not row:
            conn.close()
            await update.message.reply_text("❌ المستخدم غير موجود.", reply_markup=back_kb("admin_menu"))
            return True
        # جلب المفتاح المرتبط
        krow = conn.execute(
            "SELECT key_code, used_at, used_by_ip FROM license_keys WHERE used_by_uid=?",
            (str(row['telegram_id']),)
        ).fetchone()
        conn.close()
        ...
        if krow:
            key_info = (
                f"🔑  المفتاح     : `{krow['key_code']}`\n"
                f"📅  استخدم في   : {krow['used_at'] or '—'}\n"
                f"🌐  IP عند الاستخدام: `{krow['used_by_ip'] or '—'}`"
            )
        else:
            key_info = "🔑  المفتاح     : ❌ لم يستخدم مفتاحاً"
        await update.message.reply_text(
            f"👤 *معلومات المستخدم*\n"
            f"━━━━━━━━━━━━━━━━\n"
            ...
            f"📅  أول ظهور   : {row['first_seen']}\n"
            f"🕐  آخر ظهور   : {row['last_seen']}\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"{key_info}",
            parse_mode="Markdown",
            reply_markup=back_kb("admin_menu"),
    ...
    if action == "admin_broadcast":
        pending.pop(uid, None)
        conn   = get_db()
        users  = conn.execute("SELECT telegram_id FROM bot_visitors").fetchall()
        conn.close()
        sent = failed = 0
        for u in users:
            try:
                await ctx.bot.send_message(chat_id=int(u['telegram_id']), text=f"📢 {text}")
                sent += 1
            except Exception:
                failed += 1
        await update.message.reply_text(
...
async def auto_start_proxies():
    await asyncio.sleep(3)
    for port in PROXY_PORTS:
        if proxy_status(port) != "active":
            ok = proxy_action(port, "start")
            log.info(f"Auto-start proxy port {port}: {'OK' if ok else 'FAILED'}")

async def post_init(application):
    asyncio.create_task(periodic_cleanup_loop())
    asyncio.create_task(auto_start_proxies())
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_button, pattern="^(admin_|)admin"))
